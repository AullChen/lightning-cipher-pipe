...
帧处理和源端单块生命周期预留 `8 × maxFrameBytes + 2 × maxPlainBytes + 65536` 字节，覆盖显式载荷、帧与密码处理副本和验证缓冲；ZSTD 另计原生工作区，详见[压缩与启动示例](compression.md)。控制消息预留 256 KiB。FileSink 的索引预算单独配置。这些是应用缓冲上限，不是 JVM RSS 承诺；TLS、线程栈和运行时仍有固定开销。
