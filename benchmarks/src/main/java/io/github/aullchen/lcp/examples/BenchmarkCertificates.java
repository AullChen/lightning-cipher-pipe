...
final class BenchmarkCertificates {
    ...
    static Identity ca() throws Exception {
        var pair = pair(); var name = new X500Name("CN=LCP ephemeral benchmark CA");
    ...
    static Identity leaf(Identity ca, String node, boolean client, boolean duplicate, boolean expired, String dns) throws Exception {
        ...
        var builder = new JcaX509v3CertificateBuilder(issuer, new BigInteger(128, new SecureRandom()), Date.from(Instant.now().minusSeconds(3600)), Date.from(Instant.now().plusSeconds(expired ? -60 : 3600)), new X500Name("CN=LCP ephemeral benchmark node"), pair.getPublic());
